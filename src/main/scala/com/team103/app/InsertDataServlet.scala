...
class InsertDataServlet extends AircheckStack with JacksonJsonSupport with DatabaseSessionSupport{
  ...
  post("/symptoms") {
    try {
      ...
      val selectCartoDB = createURL(lat,long,5,"Select",11)
  ...
  private def sendCartoDB(query:String){
    val result = http("Getting Len")
    .get("https://balaber.cartodb.com/api/v2/sql")
    .queryParam("q", query)
    .queryParam("api_key", ApiValues.API_TABLE)
